...
static Logger log4j = Logger.getLogger(DateTimeData.class);
  ...
  public static String now(ConnectionProvider connectionProvider, String formatPattern)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      QueryTimeOutUtil.getInstance().setQueryTimeOut(st, SessionInfo.getQueryProfile());
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, formatPattern);

      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "now");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
