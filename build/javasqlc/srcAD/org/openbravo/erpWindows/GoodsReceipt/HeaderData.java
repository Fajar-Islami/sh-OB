...
Select for auxiliar field
 */
  public static String selectAux9655D5D7E29B4B2C80DF63CBB011148D(ConnectionProvider connectionProvider, String M_INOUT_ID)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE M_InOut" +
      "        SET AD_Org_ID = (?) , DocumentNo = (?) , M_Condition_Goods_ID = (?) , C_DocType_ID = (?) , M_Warehouse_ID = (?) , C_BPartner_ID = (?) , C_BPartner_Location_ID = (?) , MovementDate = TO_DATE(?) , DateAcct = TO_DATE(?) , C_Order_ID = (?) , POReference = (?) , Description = (?) , DocStatus = (?) , AD_User_ID = (?) , CreateFrom = (?) , GenerateTo = (?) , Process_Goods_Java = (?) , DocAction = (?) , Posted = (?) , SalesRep_ID = (?) , C_Charge_ID = (?) , ChargeAmt = TO_NUMBER(?) , DeliveryRule = (?) , DeliveryViaRule = (?) , FreightAmt = TO_NUMBER(?) , FreightCostRule = (?) , PriorityRule = (?) , MovementType = (?) , M_Shipper_ID = (?) , NoPackages = TO_NUMBER(?) , C_Activity_ID = (?) , C_Campaign_ID = (?) , AD_OrgTrx_ID = (?) , M_FreightCategory_ID = (?) , Calculate_Freight = (?) , Freight_Currency_ID = (?) , C_Project_ID = (?) , C_Costcenter_ID = (?) , A_Asset_ID = (?) , User1_ID = (?) , User2_ID = (?) , DateOrdered = TO_DATE(?) , Islogistic = (?) , ShipDate = TO_TIMESTAMP(?, ?) , TrackingNo = (?) , PickDate = TO_TIMESTAMP(?, ?) , GenerateLines = (?) , C_Invoice_ID = (?) , Delivery_Location_ID = (?) , RM_Receipt_PickEdit = (?) , RM_Shipment_Pickedit = (?) , Processed = (?) , IsPrinted = (?) , IsSOTrx = (?) , DatePrinted = TO_DATE(?) , Processing = (?) , M_InOut_ID = (?) , IsActive = (?) , AD_Client_ID = (?) , UpdateLines = (?) , updated = now(), updatedby = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO M_InOut " +
      "        (AD_Org_ID, DocumentNo, M_Condition_Goods_ID, C_DocType_ID, M_Warehouse_ID, C_BPartner_ID, C_BPartner_Location_ID, MovementDate, DateAcct, C_Order_ID, POReference, Description, DocStatus, AD_User_ID, CreateFrom, GenerateTo, Process_Goods_Java, DocAction, Posted, SalesRep_ID, C_Charge_ID, ChargeAmt, DeliveryRule, DeliveryViaRule, FreightAmt, FreightCostRule, PriorityRule, MovementType, M_Shipper_ID, NoPackages, C_Activity_ID, C_Campaign_ID, AD_OrgTrx_ID, M_FreightCategory_ID, Calculate_Freight, Freight_Currency_ID, C_Project_ID, C_Costcenter_ID, A_Asset_ID, User1_ID, User2_ID, DateOrdered, Islogistic, ShipDate, TrackingNo, PickDate, GenerateLines, C_Invoice_ID, Delivery_Location_ID, RM_Receipt_PickEdit, RM_Shipment_Pickedit, Processed, IsPrinted, IsSOTrx, DatePrinted, Processing, M_InOut_ID, IsActive, AD_Client_ID, UpdateLines, created, createdby, updated, updatedBy)" +
