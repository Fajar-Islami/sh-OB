...
Select for auxiliar field
 */
  public static String selectAux66407108B92F49CBB40135E149FC3B46(ConnectionProvider connectionProvider, String c_order_id)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "         select count(c_orderline_id) from c_orderline ol where ol.c_order_id = ?  and ol.m_inoutline_id is null ";
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_Order" +
      "        SET DocAction = (?) , RM_ReceiveMaterials = (?) , RM_AddOrphanLine = (?) , RM_PickFromShipment = (?) , RM_CreateInvoice = (?) , AD_Org_ID = (?) , DocStatus = (?) , C_DocTypeTarget_ID = (?) , GrandTotal = TO_NUMBER(?) , TotalLines = TO_NUMBER(?) , C_Currency_ID = (?) , DocumentNo = (?) , C_Return_Reason_ID = (?) , DateOrdered = TO_DATE(?) , C_BPartner_ID = (?) , C_BPartner_Location_ID = (?) , M_Warehouse_ID = (?) , InvoiceRule = (?) , C_PaymentTerm_ID = (?) , Description = (?) , FIN_Paymentmethod_ID = (?) , M_PriceList_ID = (?) , C_Project_ID = (?) , C_Activity_ID = (?) , C_Campaign_ID = (?) , AD_OrgTrx_ID = (?) , User1_ID = (?) , User2_ID = (?) , M_Shipper_ID = (?) , DeliveryViaRule = (?) , FreightAmt = TO_NUMBER(?) , BillTo_ID = (?) , SalesRep_ID = (?) , DateAcct = TO_DATE(?) , DatePromised = TO_DATE(?) , IsPrinted = (?) , IsInvoiced = (?) , IsDelivered = (?) , IsActive = (?) , AD_Client_ID = (?) , C_Order_ID = (?) , C_DocType_ID = (?) , CopyFromPO = (?) , Incotermsdescription = (?) , C_Incoterms_ID = (?) , Deliverynotes = (?) , Delivery_Location_ID = (?) , Generatetemplate = (?) , IsSelfService = (?) , DropShip_Location_ID = (?) , CopyFrom = (?) , DropShip_BPartner_ID = (?) , DropShip_User_ID = (?) , IsSelected = (?) , IsTaxIncluded = (?) , Posted = (?) , IsDiscountPrinted = (?) , PaymentRule = (?) , IsSOTrx = (?) , FreightCostRule = (?) , DeliveryRule = (?) , DatePrinted = TO_DATE(?) , Processed = (?) , C_Charge_ID = (?) , POReference = (?) , ChargeAmt = TO_NUMBER(?) , AD_User_ID = (?) , Processing = (?) , PriorityRule = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_Order.C_Order_ID = ? " +
      "        AND C_Order.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO C_Order " +
      "        (DocAction, RM_ReceiveMaterials, RM_AddOrphanLine, RM_PickFromShipment, RM_CreateInvoice, AD_Org_ID, DocStatus, C_DocTypeTarget_ID, GrandTotal, TotalLines, C_Currency_ID, DocumentNo, C_Return_Reason_ID, DateOrdered, C_BPartner_ID, C_BPartner_Location_ID, M_Warehouse_ID, InvoiceRule, C_PaymentTerm_ID, Description, FIN_Paymentmethod_ID, M_PriceList_ID, C_Project_ID, C_Activity_ID, C_Campaign_ID, AD_OrgTrx_ID, User1_ID, User2_ID, M_Shipper_ID, DeliveryViaRule, FreightAmt, BillTo_ID, SalesRep_ID, DateAcct, DatePromised, IsPrinted, IsInvoiced, IsDelivered, IsActive, AD_Client_ID, C_Order_ID, C_DocType_ID, CopyFromPO, Incotermsdescription, C_Incoterms_ID, Deliverynotes, Delivery_Location_ID, Generatetemplate, IsSelfService, DropShip_Location_ID, CopyFrom, DropShip_BPartner_ID, DropShip_User_ID, IsSelected, IsTaxIncluded, Posted, IsDiscountPrinted, PaymentRule, IsSOTrx, FreightCostRule, DeliveryRule, DatePrinted, Processed, C_Charge_ID, POReference, ChargeAmt, AD_User_ID, Processing, PriorityRule, created, createdby, updated, updatedBy)" +
