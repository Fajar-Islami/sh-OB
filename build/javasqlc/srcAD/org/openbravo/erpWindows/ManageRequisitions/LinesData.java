 ...
 */
  public static LinesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mRequisitionId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "M_RequisitionLine.Aumqty, " +
      "M_RequisitionLine.C_Aum, " +
      "(CASE WHEN M_RequisitionLine.C_Aum IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL4.Name IS NULL THEN TO_CHAR(table4.Name) ELSE TO_CHAR(tableTRL4.Name) END)), ''))),'') ) END) AS C_AumR, " +
      "M_RequisitionLine.Qty, " +
      "M_RequisitionLine.C_UOM_ID, " +
      "(CASE WHEN M_RequisitionLine.C_UOM_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL6.Name IS NULL THEN TO_CHAR(table6.Name) ELSE TO_CHAR(tableTRL6.Name) END)), ''))),'') ) END) AS C_UOM_IDR, " +
      "M_RequisitionLine.C_BPartner_ID, " +
      "(CASE WHEN M_RequisitionLine.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "M_RequisitionLine.M_PriceList_ID, " +
      "(CASE WHEN M_RequisitionLine.M_PriceList_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table9.Name), ''))),'') ) END) AS M_PriceList_IDR, " +
      "M_RequisitionLine.PriceActual, " +
      "M_RequisitionLine.LineNetAmt, " +
      "M_RequisitionLine.C_Currency_ID, " +
      "(CASE WHEN M_RequisitionLine.C_Currency_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
      ...
      "(CASE WHEN M_RequisitionLine.M_Product_Uom_Id IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL13.Name IS NULL THEN TO_CHAR(table13.Name) ELSE TO_CHAR(tableTRL13.Name) END)), ''))),'') ) END) AS M_Product_Uom_IdR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM M_RequisitionLine left join (select M_Product_ID, Name from M_Product) table1 on (M_RequisitionLine.M_Product_ID = table1.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL1 on (table1.M_Product_ID = tableTRL1.M_Product_ID and tableTRL1.AD_Language = ?)  left join (select M_AttributeSetInstance_ID, Description from M_AttributeSetInstance) table3 on (M_RequisitionLine.M_AttributeSetInstance_ID = table3.M_AttributeSetInstance_ID) left join (select C_UOM_ID, Name from C_UOM) table4 on (M_RequisitionLine.C_Aum =  table4.C_UOM_ID) left join (select C_UOM_ID,AD_Language, Name from C_UOM_TRL) tableTRL4 on (table4.C_UOM_ID = tableTRL4.C_UOM_ID and tableTRL4.AD_Language = ?)  left join (select C_UOM_ID, Name from C_UOM) table6 on (M_RequisitionLine.C_UOM_ID = table6.C_UOM_ID) left join (select C_UOM_ID,AD_Language, Name from C_UOM_TRL) tableTRL6 on (table6.C_UOM_ID = tableTRL6.C_UOM_ID and tableTRL6.AD_Language = ?)  left join (select C_BPartner_ID, Name from C_BPartner) table8 on (M_RequisitionLine.C_BPartner_ID = table8.C_BPartner_ID) left join (select M_PriceList_ID, Name from M_PriceList) table9 on (M_RequisitionLine.M_PriceList_ID = table9.M_PriceList_ID) left join (select C_Currency_ID, ISO_Code from C_Currency) table10 on (M_RequisitionLine.C_Currency_ID = table10.C_Currency_ID) left join (select AD_User_ID, Name from AD_User) table11 on (M_RequisitionLine.Lockedby =  table11.AD_User_ID) left join (select M_Product_Uom_Id, C_UOM_ID from M_Product_Uom) table12 on (M_RequisitionLine.M_Product_Uom_Id = table12.M_Product_Uom_Id) left join (select C_UOM_ID, Name from C_UOM) table13 on (table12.C_UOM_ID = table13.C_UOM_ID) left join (select C_UOM_ID,AD_Language, Name from C_UOM_TRL) tableTRL13 on (table13.C_UOM_ID = tableTRL13.C_UOM_ID and tableTRL13.AD_Language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static LinesData[] set(String mRequisitionId, String cBpartnerId, String cBpartnerIdr, String cUomId, String mProductUomId, String quantityorder, String mAttributesetinstanceId, String mAttributesetinstanceIdr, String reqstatus, String orderedqty, String description, String changestatus, String internalnotes, String suppliernotes, String needbydate, String priceactual, String discount, String cCurrencyId, String lockedby, String lockqty, String lockprice, String mPricelistId, String lockdate, String lockcause, String line, String grossAmt, String mRequisitionlineId, String adClientId, String adOrgId, String isactive, String createdby, String createdbyr, String updatedby, String updatedbyr, String mProductId, String mProductIdr, String qty, String pricelist, String linenetamt, String grossUnitPrice, String aumqty, String cAum)    throws ServletException {
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET Line = TO_NUMBER(?) , Needbydate = TO_DATE(?) , M_Product_ID = (?) , M_AttributeSetInstance_ID = (?) , Aumqty = TO_NUMBER(?) , C_Aum = (?) , Qty = TO_NUMBER(?) , C_UOM_ID = (?) , C_BPartner_ID = (?) , M_PriceList_ID = (?) , PriceActual = TO_NUMBER(?) , LineNetAmt = TO_NUMBER(?) , C_Currency_ID = (?) , Description = (?) , PriceList = TO_NUMBER(?) , Discount = TO_NUMBER(?) , Gross_Unit_Price = TO_NUMBER(?) , Gross_Amt = TO_NUMBER(?) , Internalnotes = (?) , Suppliernotes = (?) , Lockedby = (?) , Lockqty = TO_NUMBER(?) , Lockprice = TO_NUMBER(?) , QuantityOrder = TO_NUMBER(?) , M_Product_Uom_Id = (?) , Changestatus = (?) , Lockcause = (?) , Orderedqty = TO_NUMBER(?) , Lockdate = TO_DATE(?) , Reqstatus = (?) , M_Requisition_ID = (?) , IsActive = (?) , AD_Org_ID = (?) , AD_Client_ID = (?) , M_Requisitionline_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE M_RequisitionLine.M_Requisitionline_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (Line, Needbydate, M_Product_ID, M_AttributeSetInstance_ID, Aumqty, C_Aum, Qty, C_UOM_ID, C_BPartner_ID, M_PriceList_ID, PriceActual, LineNetAmt, C_Currency_ID, Description, PriceList, Discount, Gross_Unit_Price, Gross_Amt, Internalnotes, Suppliernotes, Lockedby, Lockqty, Lockprice, QuantityOrder, M_Product_Uom_Id, Changestatus, Lockcause, Orderedqty, Lockdate, Reqstatus, M_Requisition_ID, IsActive, AD_Org_ID, AD_Client_ID, M_Requisitionline_ID, created, createdby, updated, updatedBy)" +
      "        VALUES (TO_NUMBER(?), TO_DATE(?), (?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), TO_DATE(?), (?), (?), (?), (?), (?), (?), now(), ?, now(), ?)";
