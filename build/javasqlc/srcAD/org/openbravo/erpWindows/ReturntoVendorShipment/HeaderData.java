 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE M_InOut" +
      "        SET RM_Shipment_Pickedit = (?) , AD_Org_ID = (?) , C_DocType_ID = (?) , M_Condition_Goods_ID = (?) , DocumentNo = (?) , POReference = (?) , C_BPartner_ID = (?) , C_BPartner_Location_ID = (?) , MovementDate = TO_DATE(?) , DateAcct = TO_DATE(?) , M_Warehouse_ID = (?) , C_Order_ID = (?) , Description = (?) , DocStatus = (?) , AD_User_ID = (?) , CreateFrom = (?) , GenerateTo = (?) , DocAction = (?) , Posted = (?) , SalesRep_ID = (?) , C_Charge_ID = (?) , ChargeAmt = TO_NUMBER(?) , DeliveryRule = (?) , DeliveryViaRule = (?) , FreightAmt = TO_NUMBER(?) , FreightCostRule = (?) , PriorityRule = (?) , MovementType = (?) , M_Shipper_ID = (?) , NoPackages = TO_NUMBER(?) , M_FreightCategory_ID = (?) , Calculate_Freight = (?) , Freight_Currency_ID = (?) , C_Project_ID = (?) , C_Activity_ID = (?) , C_Campaign_ID = (?) , AD_OrgTrx_ID = (?) , User1_ID = (?) , User2_ID = (?) , DateOrdered = TO_DATE(?) , C_Invoice_ID = (?) , TrackingNo = (?) , PickDate = TO_TIMESTAMP(?, ?) , RM_Receipt_PickEdit = (?) , AD_Client_ID = (?) , GenerateLines = (?) , Processing = (?) , ShipDate = TO_TIMESTAMP(?, ?) , DatePrinted = TO_DATE(?) , Processed = (?) , M_InOut_ID = (?) , Islogistic = (?) , UpdateLines = (?) , IsActive = (?) , IsPrinted = (?) , Delivery_Location_ID = (?) , IsSOTrx = (?) , updated = now(), updatedby = ? " +
      "        WHERE M_InOut.M_InOut_ID = ? " +
      "        AND M_InOut.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO M_InOut " +
      "        (RM_Shipment_Pickedit, AD_Org_ID, C_DocType_ID, M_Condition_Goods_ID, DocumentNo, POReference, C_BPartner_ID, C_BPartner_Location_ID, MovementDate, DateAcct, M_Warehouse_ID, C_Order_ID, Description, DocStatus, AD_User_ID, CreateFrom, GenerateTo, DocAction, Posted, SalesRep_ID, C_Charge_ID, ChargeAmt, DeliveryRule, DeliveryViaRule, FreightAmt, FreightCostRule, PriorityRule, MovementType, M_Shipper_ID, NoPackages, M_FreightCategory_ID, Calculate_Freight, Freight_Currency_ID, C_Project_ID, C_Activity_ID, C_Campaign_ID, AD_OrgTrx_ID, User1_ID, User2_ID, DateOrdered, C_Invoice_ID, TrackingNo, PickDate, RM_Receipt_PickEdit, AD_Client_ID, GenerateLines, Processing, ShipDate, DatePrinted, Processed, M_InOut_ID, Islogistic, UpdateLines, IsActive, IsPrinted, Delivery_Location_ID, IsSOTrx, created, createdby, updated, updatedBy)" +
