...
Select for auxiliar field
 */
  public static String selectAux44F99FD664A64A9683694FE63672BD55(ConnectionProvider connectionProvider, String M_Product_ID)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
 ...
 */
  public static String selectAux9E0F5BB107EE463B8E4F18395952D8F2(ConnectionProvider connectionProvider, String c_order_id)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        select coalesce(max(dt.docbasetype), 'POO') from c_order ord join c_doctype dt on (ord.c_doctypetarget_id = dt.c_doctype_id)" +
...
Select for auxiliar field
 */
  public static String selectAuxF12E4458B64640EC9200B936D58337ED(ConnectionProvider connectionProvider, String M_Product_ID)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_OrderLine" +
      "        SET Line = TO_NUMBER(?) , M_Product_ID = (?) , Aumqty = TO_NUMBER(?) , C_Aum = (?) , QtyOrdered = TO_NUMBER(?) , C_UOM_ID = (?) , M_Inoutline_ID = (?) , M_AttributeSetInstance_ID = (?) , C_Return_Reason_ID = (?) , PriceActual = TO_NUMBER(?) , Gross_Unit_Price = TO_NUMBER(?) , LineNetAmt = TO_NUMBER(?) , Line_Gross_Amount = TO_NUMBER(?) , C_Tax_ID = (?) , PriceList = TO_NUMBER(?) , GrossPriceList = TO_NUMBER(?) , Discount = TO_NUMBER(?) , Description = (?) , Taxbaseamt = TO_NUMBER(?) , Iseditlinenetamt = (?) , CANCELPRICEAD = (?) , PriceStd = TO_NUMBER(?) , grosspricestd = TO_NUMBER(?) , M_Warehouse_ID = (?) , QtyReserved = TO_NUMBER(?) , QtyDelivered = TO_NUMBER(?) , QtyInvoiced = TO_NUMBER(?) , M_Shipper_ID = (?) , DateOrdered = TO_DATE(?) , DatePromised = TO_DATE(?) , C_BPartner_ID = (?) , FreightAmt = TO_NUMBER(?) , C_Charge_ID = (?) , C_BPartner_Location_ID = (?) , QuantityOrder = TO_NUMBER(?) , M_Product_Uom_Id = (?) , Manage_Prereservation = (?) , Quotationline_ID = (?) , AD_Org_ID = (?) , C_Project_ID = (?) , C_Costcenter_ID = (?) , A_Asset_ID = (?) , User1_ID = (?) , User2_ID = (?) , Explode = (?) , BOM_Parent_ID = (?) , S_ResourceAssignment_ID = (?) , DateInvoiced = TO_DATE(?) , DateDelivered = TO_DATE(?) , SO_Res_Status = (?) , Manage_Reservation = (?) , Ref_OrderLine_ID = (?) , C_Order_ID = (?) , IsActive = (?) , AD_Client_ID = (?) , C_OrderLine_ID = (?) , M_Warehouse_Rule_ID = (?) , C_Currency_ID = (?) , IsDescription = (?) , M_Offer_ID = (?) , PriceLimit = TO_NUMBER(?) , C_Order_Discount_ID = (?) , Create_Reservation = (?) , DirectShip = (?) , ChargeAmt = TO_NUMBER(?) , updated = now(), updatedby = ? " +
      "        WHERE C_OrderLine.C_OrderLine_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (Line, M_Product_ID, Aumqty, C_Aum, QtyOrdered, C_UOM_ID, M_Inoutline_ID, M_AttributeSetInstance_ID, C_Return_Reason_ID, PriceActual, Gross_Unit_Price, LineNetAmt, Line_Gross_Amount, C_Tax_ID, PriceList, GrossPriceList, Discount, Description, Taxbaseamt, Iseditlinenetamt, CANCELPRICEAD, PriceStd, grosspricestd, M_Warehouse_ID, QtyReserved, QtyDelivered, QtyInvoiced, M_Shipper_ID, DateOrdered, DatePromised, C_BPartner_ID, FreightAmt, C_Charge_ID, C_BPartner_Location_ID, QuantityOrder, M_Product_Uom_Id, Manage_Prereservation, Quotationline_ID, AD_Org_ID, C_Project_ID, C_Costcenter_ID, A_Asset_ID, User1_ID, User2_ID, Explode, BOM_Parent_ID, S_ResourceAssignment_ID, DateInvoiced, DateDelivered, SO_Res_Status, Manage_Reservation, Ref_OrderLine_ID, C_Order_ID, IsActive, AD_Client_ID, C_OrderLine_ID, M_Warehouse_Rule_ID, C_Currency_ID, IsDescription, M_Offer_ID, PriceLimit, C_Order_Discount_ID, Create_Reservation, DirectShip, ChargeAmt, created, createdby, updated, updatedBy)" +
      "        VALUES (TO_NUMBER(?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), (?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_NUMBER(?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_DATE(?), TO_DATE(?), (?), TO_NUMBER(?), (?), (?), TO_NUMBER(?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), TO_DATE(?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), now(), ?, now(), ?)";
