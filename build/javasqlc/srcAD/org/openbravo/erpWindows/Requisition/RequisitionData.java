 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE M_Requisition" +
      "        SET AD_Org_ID = (?) , DocumentNo = (?) , Description = (?) , C_BPartner_ID = (?) , AD_User_ID = (?) , M_PriceList_ID = (?) , C_Currency_ID = (?) , DocAction = (?) , DocStatus = (?) , M_Requisition_ID = (?) , Processed = (?) , Createpo = (?) , IsActive = (?) , AD_Client_ID = (?) , Processing = (?) , updated = now(), updatedby = ? " +
      "        WHERE M_Requisition.M_Requisition_ID = ? " +
      "        AND M_Requisition.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Org_ID, DocumentNo, Description, C_BPartner_ID, AD_User_ID, M_PriceList_ID, C_Currency_ID, DocAction, DocStatus, M_Requisition_ID, Processed, Createpo, IsActive, AD_Client_ID, Processing, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), now(), ?, now(), ?)";
