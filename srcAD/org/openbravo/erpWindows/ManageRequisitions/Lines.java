    ...
    if (command.contains("BUTTON")) {
     ...
     SessionInfo.setQueryProfile("manualProcess");
      ...
      if (command.contains("1004400002")) {
        ...
        if (securedProcess || explicitAccess.contains("1004400002")) {
          classInfo.type = "P";
...
vars.getGlobalVariable("inpmRequisitionId", windowId + "|M_Requisition_ID");
  ...
  private void refreshParentSession(VariablesSecureApp vars, String strPM_Requisition_ID) throws IOException,ServletException {
      ...
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Org_ID", data[0].adOrgId);    vars.setSessionValue(windowId + "|M_PriceList_ID", data[0].mPricelistId);    vars.setSessionValue(windowId + "|C_Currency_ID", data[0].cCurrencyId);    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].adClientId);    vars.setSessionValue(windowId + "|M_Requisition_ID", data[0].mRequisitionId);
...
xmlDocument.setParameter("grid_Default", selectedRow);
  ...
  private void printPageEdit(HttpServletResponse response, HttpServletRequest request, VariablesSecureApp vars,boolean _boolNew, String _M_Requisitionline_ID, String strPM_Requisition_ID, TableSQLData tableSQL)
    throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: edit");

    // copy params to variables as they will be modified later
    boolean boolNew = _boolNew;
    String strM_Requisitionline_ID = _M_Requisitionline_ID;
...
String strParamNeedbydate_f = vars.getSessionValue(tabId + "|paramNeedbydate_f");
    ...
    if (dataField==null) {
      ...
      if (dataField.getField("mRequisitionlineId") == null || dataField.getField("mRequisitionlineId").equals("")) {
        ...
        String strGROSSPRICE = LinesData.selectAux2252C06D4EA7419FA38E7B5565EF411C(this, ((dataField!=null)?dataField.getField("mPricelistId"):((data==null || data.length==0)?"":data[0].getField("mPricelistId"))), strPM_Requisition_ID);
    vars.setSessionValue(windowId + "|GROSSPRICE", strGROSSPRICE);
        String strATTRIBUTESET = LinesData.selectAux485726718FCD47AFB101C59FC1BD365E(this, ((dataField!=null)?dataField.getField("mProductId"):((data==null || data.length==0)?"":data[0].getField("mProductId"))));
    vars.setSessionValue(windowId + "|ATTRIBUTESET", strATTRIBUTESET);
        String strATTRSETVALUETYPE = LinesData.selectAuxACA00D8708854C3E80E50FE99724C4E8(this, ((dataField!=null)?dataField.getField("mProductId"):((data==null || data.length==0)?"":data[0].getField("mProductId"))));
