...
vars.getGlobalVariable("inpglJournalId", windowId + "|GL_Journal_ID");
  ...
  private void refreshParentSession(VariablesSecureApp vars, String strPGL_Journal_ID) throws IOException,ServletException {
      ...
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Org_ID", data[0].adOrgId);    vars.setSessionValue(windowId + "|C_AcctSchema_ID", data[0].cAcctschemaId);    vars.setSessionValue(windowId + "|C_DocType_ID", data[0].cDoctypeId);    vars.setSessionValue(windowId + "|DateDoc", data[0].datedoc);    vars.setSessionValue(windowId + "|DateAcct", data[0].dateacct);    vars.setSessionValue(windowId + "|C_Period_ID", data[0].cPeriodId);    vars.setSessionValue(windowId + "|C_Currency_ID", data[0].cCurrencyId);    vars.setSessionValue(windowId + "|CurrencyRateType", data[0].currencyratetype);    vars.setSessionValue(windowId + "|CurrencyRate", data[0].currencyrate);    vars.setSessionValue(windowId + "|Posted", data[0].posted);    vars.setSessionValue(windowId + "|GL_Category_ID", data[0].glCategoryId);    vars.setSessionValue(windowId + "|GL_Journal_ID", data[0].glJournalId);    vars.setSessionValue(windowId + "|Processed", data[0].processed);    vars.setSessionValue(windowId + "|GL_JournalBatch_ID", data[0].glJournalbatchId);    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].adClientId);
      vars.setSessionValue(windowId + "|GL_Journal_ID", strPGL_Journal_ID); //to ensure key parent is set for EM_* cols

      FieldProvider dataField = null; // Define this so that auxiliar inputs using SQL will work
      
      vars.setSessionValue(windowId + "|DESCRIPTION0", HeaderData.selectAux12(this, Utility.getContext(this, vars, "GL_JOURNALBATCH_ID", "132")));
      
      vars.setSessionValue(windowId + "|HeaderDateAcct", ((dataField!=null)?dataField.getField("dateacct"):((data==null || data.length==0)?"":data[0].getField("dateacct"))));
      
      vars.setSessionValue(windowId + "|HeaderDateDoc", Utility.getContext(this, vars, "DateDoc", "132"));
      
      vars.setSessionValue(windowId + "|generalLedgerCurrency", HeaderData.selectAux9F7DE5D4045A4391A93E1444C6D921C7(this, Utility.getContext(this, vars, "c_acctschema_id", "132")));
      
      vars.setSessionValue(windowId + "|Parent_AD_Org", ((dataField!=null)?dataField.getField("adOrgId"):((data==null || data.length==0)?"":data[0].getField("adOrgId"))));
...
String strParamLine_f = vars.getSessionValue(tabId + "|paramLine_f");
    ...
    vars.setSessionValue(windowId + "|Posted", strPosted);
        String strNumberOfAcctSchemas = LinesData.selectAux8482E2A2C9D74EA592CE8AF25AA9364C(this, ((dataField!=null)?dataField.getField("adOrgId"):((data==null || data.length==0)?"":data[0].getField("adOrgId"))));
    ...
    vars.setSessionValue(windowId + "|DOCBASETYPE", strDOCBASETYPE);
        String strisSOTrx = LinesData.selectAuxFAFC5E1A98E243BE9473EF29984BC8C8(this, ((dataField!=null)?dataField.getField("finPaymentId"):((data==null || data.length==0)?"":data[0].getField("finPaymentId"))));
