...
PInstanceProcessData.insertPInstanceParam(this, pinstance, "20", "C_AcctSchema_ID", strcAcctschemaId, vars.getClient(), vars.getOrg(), vars.getUser());
  ...
  private void refreshParentSession(VariablesSecureApp vars, String strPM_Product_Category_ID) throws IOException,ServletException {
      ...
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Org_ID", data[0].adOrgId);    vars.setSessionValue(windowId + "|M_Product_Category_ID", data[0].mProductCategoryId);    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].adClientId);
  ...
  private String getParentID(VariablesSecureApp vars, String strM_Product_Category_Acct_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|C_AcctSchema_ID", data[0].getField("cAcctschemaId"));    vars.setSessionValue(windowId + "|M_Product_Category_Acct_ID", data[0].getField("mProductCategoryAcctId"));    vars.setSessionValue(windowId + "|M_Product_Category_ID", data[0].getField("mProductCategoryId"));    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));
    }
...
//    }
    ...
    if (vars.getLanguage().equals("en_US")) xmlDocument.setParameter("parent", AccountingData.selectParent(this, vars.getLanguage(), strPM_Product_Category_ID));
    else xmlDocument.setParameter("parent", AccountingData.selectParentTrl(this, vars.getLanguage(), strPM_Product_Category_ID));
...
xmlDocument.setParameter("grid_Default", selectedRow);
  ...
  private void printPageEdit(HttpServletResponse response, HttpServletRequest request, VariablesSecureApp vars,boolean _boolNew, String _M_Product_Category_Acct_ID, String strPM_Product_Category_ID, TableSQLData tableSQL)
    throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: edit");

    // copy params to variables as they will be modified later
    boolean boolNew = _boolNew;
    String strM_Product_Category_Acct_ID = _M_Product_Category_Acct_ID;
