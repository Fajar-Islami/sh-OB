...
vars.getGlobalVariable("inpmrpRunProductionId", windowId + "|MRP_Run_Production_ID");
  ...
  private void refreshParentSession(VariablesSecureApp vars, String strPMRP_Run_Production_ID) throws IOException,ServletException {
      ...
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Org_ID", data[0].adOrgId);    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].adClientId);    vars.setSessionValue(windowId + "|MRP_Run_Production_ID", data[0].mrpRunProductionId);
  ...
  private String getParentID(VariablesSecureApp vars, String strMRP_Run_Productionline_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|Inouttrxtype", data[0].getField("inouttrxtype"));    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));    vars.setSessionValue(windowId + "|MRP_Run_Productionline_ID", data[0].getField("mrpRunProductionlineId"));
    }
...
xmlDocument.setParameter("grid_Default", selectedRow);
  ...
  private void printPageEdit(HttpServletResponse response, HttpServletRequest request, VariablesSecureApp vars,boolean _boolNew, String _MRP_Run_Productionline_ID, String strPMRP_Run_Production_ID, TableSQLData tableSQL)
    throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: edit");

    // copy params to variables as they will be modified later
    boolean boolNew = _boolNew;
    String strMRP_Run_Productionline_ID = _MRP_Run_Productionline_ID;
...
comboTableData = null;
comboTableData = new ComboTableData(vars, this, "19", "C_OrderLine_ID", "", "74C2EDFE09A34C46A3831C440C410176", Utility.getReferenceableOrg(vars, (dataField!=null?dataField.getField("adOrgId"):data[0].getField("adOrgId").equals("")?vars.getOrg():data[0].getField("adOrgId"))), Utility.getContext(this, vars, "#User_Client", windowId), 0);
