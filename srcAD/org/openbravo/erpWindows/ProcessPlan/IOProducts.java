...
params.put("copyattribute", strcopyattribute);
  ...
  private void refreshParentSession(VariablesSecureApp vars, String strPMA_Sequence_ID) throws IOException,ServletException {
      ...
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|MA_Sequence_ID", data[0].maSequenceId);    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].adOrgId);    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].adClientId);
  ...
  private String getParentID(VariablesSecureApp vars, String strMA_Sequenceproduct_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|Line", data[0].getField("line"));    vars.setSessionValue(windowId + "|MA_Sequenceproduct_ID", data[0].getField("maSequenceproductId"));    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));
    }
...
xmlDocument.setParameter("grid_Default", selectedRow);
  ...
  private void printPageEdit(HttpServletResponse response, HttpServletRequest request, VariablesSecureApp vars,boolean _boolNew, String _MA_Sequenceproduct_ID, String strPMA_Sequence_ID, TableSQLData tableSQL)
    throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: edit");

    // copy params to variables as they will be modified later
    boolean boolNew = _boolNew;
    String strMA_Sequenceproduct_ID = _MA_Sequenceproduct_ID;
    ...
    if (dataField==null) {
      ...
      if (dataField.getField("maSequenceproductId") == null || dataField.getField("maSequenceproductId").equals("")) {
        ...
        String strHASSECONDUOM = IOProductsData.selectAux800038(this, ((dataField!=null)?dataField.getField("mProductId"):((data==null || data.length==0)?"":data[0].getField("mProductId"))));
