...
vars.getGlobalVariable("inpfinDoubtfulDebtRunId", windowId + "|FIN_Doubtful_Debt_Run_ID");
  ...
  private String getParentID(VariablesSecureApp vars, String strFIN_Doubtful_Debt_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));    vars.setSessionValue(windowId + "|FIN_Doubtful_Debt_ID", data[0].getField("finDoubtfulDebtId"));
    }
...
xmlDocument.setParameter("grid_Default", selectedRow);
  ...
  private void printPageEdit(HttpServletResponse response, HttpServletRequest request, VariablesSecureApp vars,boolean _boolNew, String _FIN_Doubtful_Debt_ID, String strPFIN_Doubtful_Debt_Run_ID, TableSQLData tableSQL)
    throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: edit");

    // copy params to variables as they will be modified later
    boolean boolNew = _boolNew;
    String strFIN_Doubtful_Debt_ID = _FIN_Doubtful_Debt_ID;
