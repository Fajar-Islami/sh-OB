...
params.put("action", straction);
  ...
  private ImportedBankStatementsData getEditVariables(Connection con, VariablesSecureApp vars, String strPFIN_Financial_Account_ID) throws IOException,ServletException {
    ...
    data.documentno = vars.getRequiredStringParameter("inpdocumentno");     data.cDoctypeId = vars.getRequiredStringParameter("inpcDoctypeId");     data.cDoctypeIdr = vars.getStringParameter("inpcDoctypeId_R");     data.name = vars.getRequiredStringParameter("inpname");     data.isactive = vars.getStringParameter("inpisactive", "N");     data.importdate = vars.getRequiredStringParameter("inpimportdate");     data.statementdate = vars.getRequiredStringParameter("inpstatementdate");     data.filename = vars.getStringParameter("inpfilename");     data.finReconciliationId = vars.getStringParameter("inpfinReconciliationId");     data.notes = vars.getStringParameter("inpnotes");     data.posted = vars.getRequiredStringParameter("inpposted");     data.emAprmProcessBs = vars.getRequestGlobalVariable("inpemAprmProcessBs", windowId + "|EM_APRM_Process_BS");     data.emAprmProcessBsForce = vars.getRequestGlobalVariable("inpemAprmProcessBsForce", windowId + "|EM_APRM_Process_BS_Force");     data.finFinancialAccountId = vars.getRequiredStringParameter("inpfinFinancialAccountId");     data.finBankstatementId = vars.getRequestGlobalVariable("inpfinBankstatementId", windowId + "|FIN_Bankstatement_ID");     data.processed = vars.getRequiredInputGlobalVariable("inpprocessed", windowId + "|Processed", "N");     data.processing = vars.getStringParameter("inpprocessing", "N");     data.adOrgId = vars.getRequiredGlobalVariable("inpadOrgId", windowId + "|AD_Org_ID");     data.adClientId = vars.getRequiredGlobalVariable("inpadClientId", windowId + "|AD_Client_ID"); 
      ...
      data.finFinancialAccountId = vars.getGlobalVariable("inpfinFinancialAccountId", windowId + "|FIN_Financial_Account_ID");
          ...
          vars.getGlobalVariable("inpaprmIsreconciled", windowId + "|APRM_ISRECONCILED", "");
  ...
  private String getParentID(VariablesSecureApp vars, String strFIN_Bankstatement_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|EM_APRM_Process_BS", data[0].getField("emAprmProcessBs"));    vars.setSessionValue(windowId + "|EM_APRM_Process_BS_Force", data[0].getField("emAprmProcessBsForce"));    vars.setSessionValue(windowId + "|Processed", data[0].getField("processed"));    vars.setSessionValue(windowId + "|FIN_Bankstatement_ID", data[0].getField("finBankstatementId"));    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));
    }
...
xmlDocument.setParameter("grid_Default", selectedRow);
  ...
  private void printPageEdit(HttpServletResponse response, HttpServletRequest request, VariablesSecureApp vars,boolean _boolNew, String _FIN_Bankstatement_ID, String strPFIN_Financial_Account_ID, TableSQLData tableSQL)
    throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: edit");

    // copy params to variables as they will be modified later
    boolean boolNew = _boolNew;
    String strFIN_Bankstatement_ID = _FIN_Bankstatement_ID;
...
String strParamStatementdate_f = vars.getSessionValue(tabId + "|paramStatementdate_f");
    ...
    try {
      ...
      throw new ServletException(ex);
        ...
        xmlDocument.setParameter("APRM_ISRECONCILED", strAPRM_ISRECONCILED);
...
xmlDocument.setParameter("EM_APRM_Process_BS_Force_BTNname", Utility.getButtonName(this, vars, "EC75B6F5A9504DB6B3F3356EA85F15EE", (dataField==null?data[0].getField("emAprmProcessBsForce"):dataField.getField("emAprmProcessBsForce")), "EM_APRM_Process_BS_Force_linkBTN", usedButtonShortCuts, reservedButtonShortCuts));boolean modalEM_APRM_Process_BS_Force = org.openbravo.erpCommon.utility.Utility.isModalProcess("2DDE7D3618034C38A4462B7F3456C28D"); 
xmlDocument.setParameter("EM_APRM_Process_BS_Force_Modal", modalEM_APRM_Process_BS_Force?"true":"false");
            ...
            if(data != null ) {
                if(type == 'I') {            			
                    ...
                    data.emAprmProcessBsBtn = ActionButtonDefaultData.getText(this, vars.getLanguage(), "EC75B6F5A9504DB6B3F3356EA85F15EE", data.getField("EM_APRM_Process_BS"));
                        ...
                        //BUTTON TEXT FILLING
                    data.emAprmProcessBsForceBtn = ActionButtonDefaultData.getText(this, vars.getLanguage(), "EC75B6F5A9504DB6B3F3356EA85F15EE", data.getField("EM_APRM_Process_BS_Force"));
