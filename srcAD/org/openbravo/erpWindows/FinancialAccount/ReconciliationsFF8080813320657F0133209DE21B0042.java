...
params.put("action", straction);
  ...
  private ReconciliationsFF8080813320657F0133209DE21B0042Data getEditVariables(Connection con, VariablesSecureApp vars, String strPFIN_Financial_Account_ID) throws IOException,ServletException {
    ...
    data.adOrgId = vars.getRequiredGlobalVariable("inpadOrgId", windowId + "|AD_Org_ID");     data.isactive = vars.getStringParameter("inpisactive", "N");     data.finFinancialAccountId = vars.getRequiredStringParameter("inpfinFinancialAccountId");     data.documentno = vars.getRequiredStringParameter("inpdocumentno");     data.cDoctypeId = vars.getRequiredStringParameter("inpcDoctypeId");     data.cDoctypeIdr = vars.getStringParameter("inpcDoctypeId_R");     data.dateto = vars.getRequiredStringParameter("inpdateto");     data.statementdate = vars.getRequiredStringParameter("inpstatementdate");     data.printdetailed = vars.getStringParameter("inpprintdetailed", "N");     data.emAprmPrintdetailed = vars.getRequiredStringParameter("inpemAprmPrintdetailed");    try {   data.startingbalance = vars.getRequiredNumericParameter("inpstartingbalance");  } catch (ServletException paramEx) { ex = paramEx; }    try {   data.endingbalance = vars.getRequiredNumericParameter("inpendingbalance");  } catch (ServletException paramEx) { ex = paramEx; }     data.printsummary = vars.getStringParameter("inpprintsummary", "N");     data.emAprmPrintsummary = vars.getRequiredStringParameter("inpemAprmPrintsummary");     data.docstatus = vars.getRequiredStringParameter("inpdocstatus");     data.processing = vars.getStringParameter("inpprocessing", "N");     data.processed = vars.getRequiredInputGlobalVariable("inpprocessed", windowId + "|Processed", "N");     data.posted = vars.getRequiredStringParameter("inpposted");     data.emAprmProcessReconciliation = vars.getStringParameter("inpemAprmProcessReconciliation");     data.emAprmProcessRec = vars.getStringParameter("inpemAprmProcessRec");     data.emAprmProcessRecForce = vars.getStringParameter("inpemAprmProcessRecForce");     data.adClientId = vars.getRequiredGlobalVariable("inpadClientId", windowId + "|AD_Client_ID");     data.finReconciliationId = vars.getRequestGlobalVariable("inpfinReconciliationId", windowId + "|FIN_Reconciliation_ID"); 
      ...
      data.finFinancialAccountId = vars.getGlobalVariable("inpfinFinancialAccountId", windowId + "|FIN_Financial_Account_ID");
          ...
          vars.getGlobalVariable("inpismanual", windowId + "|ISMANUAL", "");
          vars.getGlobalVariable("inpaprmLastrec", windowId + "|APRM_LASTREC", "");
  ...
  private String getParentID(VariablesSecureApp vars, String strFIN_Reconciliation_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));    vars.setSessionValue(windowId + "|Processed", data[0].getField("processed"));    vars.setSessionValue(windowId + "|FIN_Reconciliation_ID", data[0].getField("finReconciliationId"));    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));
    }
...
xmlDocument.setParameter("grid_Default", selectedRow);
  ...
  private void printPageEdit(HttpServletResponse response, HttpServletRequest request, VariablesSecureApp vars,boolean _boolNew, String _FIN_Reconciliation_ID, String strPFIN_Financial_Account_ID, TableSQLData tableSQL)
    throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: edit");

    // copy params to variables as they will be modified later
    boolean boolNew = _boolNew;
    String strFIN_Reconciliation_ID = _FIN_Reconciliation_ID;
...
String strParamStatementdate_f = vars.getSessionValue(tabId + "|paramStatementdate_f");
    ...
    try {
      ...
      throw new ServletException(ex);
        ...
        xmlDocument.setParameter("ISMANUAL", strISMANUAL);
        xmlDocument.setParameter("APRM_LASTREC", strAPRM_LASTREC);
...
xmlDocument.setParameter("EM_APRM_Process_Rec_Force_BTNname", Utility.getButtonName(this, vars, "FF8080812E443491012E443C053A001A", (dataField==null?data[0].getField("emAprmProcessRecForce"):dataField.getField("emAprmProcessRecForce")), "EM_APRM_Process_Rec_Force_linkBTN", usedButtonShortCuts, reservedButtonShortCuts));boolean modalEM_APRM_Process_Rec_Force = org.openbravo.erpCommon.utility.Utility.isModalProcess("6BF16EFC772843AC9A17552AE0B26AB7"); 
xmlDocument.setParameter("EM_APRM_Process_Rec_Force_Modal", modalEM_APRM_Process_Rec_Force?"true":"false");
            ...
            if(data != null ) {
                if(type == 'I') {            			
                    data.finReconciliationId = "";
                        ...
                        //BUTTON TEXT FILLING
                    data.emAprmProcessRecForceBtn = ActionButtonDefaultData.getText(this, vars.getLanguage(), "FF8080812E443491012E443C053A001A", data.getField("EM_APRM_Process_Rec_Force"));
