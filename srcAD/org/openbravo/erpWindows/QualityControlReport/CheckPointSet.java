...
vars.getGlobalVariable("inpmaMeasureShiftId", windowId + "|MA_Measure_Shift_ID");
  ...
  private void refreshParentSession(VariablesSecureApp vars, String strPMA_Measure_Shift_ID) throws IOException,ServletException {
      ...
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Org_ID", data[0].adOrgId);    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].adClientId);    vars.setSessionValue(windowId + "|MA_Measure_Shift_ID", data[0].maMeasureShiftId);
  ...
  private String getParentID(VariablesSecureApp vars, String strMA_Measure_Group_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));    vars.setSessionValue(windowId + "|MA_Measure_Group_ID", data[0].getField("maMeasureGroupId"));
    }
...
xmlDocument.setParameter("grid_Default", selectedRow);
  ...
  private void printPageEdit(HttpServletResponse response, HttpServletRequest request, VariablesSecureApp vars,boolean _boolNew, String _MA_Measure_Group_ID, String strPMA_Measure_Shift_ID, TableSQLData tableSQL)
    throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: edit");

    // copy params to variables as they will be modified later
    boolean boolNew = _boolNew;
    String strMA_Measure_Group_ID = _MA_Measure_Group_ID;
